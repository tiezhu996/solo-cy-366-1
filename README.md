...
>
> 历史遗留故障自愈：若机位处于 `fault` 但不存在待处理报修单（旧数据/直接改库造成的脏数据），调用恢复接口并填写处理结果时不再报 40011 卡死，而是补录一条原因留痕为“历史故障遗留（无待处理报修登记）”的 `closed` 记录（无登记人快照、处理结果/处理角色/处理时间完整留存），机位恢复空闲；响应中 `legacy=true` 标记此路径。
>
> 删除守卫：机位存在任何报修记录（待处理或已关闭历史）时，管理员删除机位返回 409 / code=40012，避免报修管理页残留孤儿记录、机位详情 404；无报修记录的机位移除保持可用（仅管理员可删除）。
